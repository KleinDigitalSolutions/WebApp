...
export default function ActivitiesCard() {
  ...
  return (
    <div className="bg-gray-900 rounded-3xl shadow-lg border border-gray-800 p-6 text-gray-200 relative">
      <div className="flex items-center mb-3">
        <span className="text-2xl mr-2">🏃‍♂️</span>
        <h3 className="text-lg font-semibold text-gray-100">Aktivität hinzufügen</h3>
      </div>
      <p className="text-gray-400 text-sm mb-4">Wähle eine Aktivität aus der Liste und füge sie deinem Tagebuch hinzu.</p>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-72 overflow-y-auto">
        {activitiesList.map((activity) => (
          <button
            key={activity.id}
            onClick={() => setSelected(activity.id)}
            className={`flex items-center px-3 py-2 rounded-2xl border shadow-sm transition-all duration-200 focus:outline-none ${selected === activity.id ? 'bg-emerald-500 text-white scale-105 border-emerald-400' : 'bg-gray-800 border-gray-700 hover:bg-gray-700 text-gray-200'}`}
          >
      ...
      {selected && (
        <div className="mt-6 flex flex-col items-center">
          ...
          <div className="text-sm text-gray-400 mb-2">MET: {activitiesList.find(a => a.id === selected)?.met}</div>
          <label className="mt-2 text-sm text-gray-300">Dauer (Minuten):
            <input
              type="number"
              min={5}
              max={300}
              ...
              className="ml-2 px-2 py-1 rounded bg-gray-800 text-gray-200 w-20 text-center border border-gray-600"
            />
          </label>
          <label className="mt-2 text-sm text-gray-300">Notiz (optional):
            <input
              type="text"
              value={note}
              onChange={e => setNote(e.target.value)}
              className="ml-2 px-2 py-1 rounded bg-gray-800 text-gray-200 w-40 border border-gray-600"
              placeholder="z.B. Strecke, Puls..."
            />
          </label>
          <div className="mt-2 text-gray-400 text-sm">Geschätzter Verbrauch: <b>{calcCalories(activitiesList.find(a => a.id === selected)?.met || 1, profile?.weight_kg || 70, duration)}</b> kcal</div>
          <button
          ...
          <button
            className="mt-2 px-4 py-2 bg-gray-600 text-white rounded-full font-semibold shadow-lg hover:bg-gray-700 transition"
      ...
      {success && (
        <div className="mt-4 text-emerald-400 font-semibold text-center">Aktivität gespeichert! 🎉</div>
      )}
    </div>
  )
}
