...
-- =====================================================
-- PART 1: Profile Table Updates (first_name, last_name)
-- =====================================================
...
-- =====================================================
-- PART 2: User-Generated Products System
-- =====================================================
...
-- Drop existing policies if they exist (to avoid conflicts)
DROP POLICY IF EXISTS "Users can view all product reviews" ON public.product_reviews;
DROP POLICY IF EXISTS "Users can insert own product reviews" ON public.product_reviews;
...
DROP POLICY IF EXISTS "Users can insert own product reports" ON public.product_reports;
DROP POLICY IF EXISTS "Users can update own product reports" ON public.product_reports;
DROP POLICY IF EXISTS "Users can delete own product reports" ON public.product_reports;
...
-- Enable realtime for products table (optional, only if not already added)
DO $$
BEGIN
  -- Check and add products table to realtime
  IF NOT EXISTS (
  ...
  -- Check and add product_reviews table to realtime
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables 
    WHERE pubname = 'supabase_realtime' 
  ...
  -- Check and add product_reports table to realtime
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables 
    WHERE pubname = 'supabase_realtime' 
...
-- 1. Profile names (first_name, last_name) will work
-- 2. User-generated products system will be available
