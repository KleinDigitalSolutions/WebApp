-- =============================================
-- FIX FÜR RLS POLICIES - A.N.D LETICS (v3)
-- =============================================
-- Dieses SQL löst die 401/400 Fehler beim Registrieren

-- 1. Alle relevanten Policies löschen (falls vorhanden)
DROP POLICY IF EXISTS "Enable insert for authenticated users only" ON profiles;
DROP POLICY IF EXISTS "Users can view own profile" ON profiles;
DROP POLICY IF EXISTS "Users can update own profile" ON profiles;
DROP POLICY IF EXISTS "Users can delete own profile" ON profiles;
DROP POLICY IF EXISTS "Users can insert own profile" ON profiles; -- (Legacy, falls noch vorhanden)
